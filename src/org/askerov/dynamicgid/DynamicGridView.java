...
import com.example.horizontalimageview.*;
...
public class DynamicGridView extends GridView {
	...
	private void startWobbleAnimation(int position) {
	...
	private void restartWobble() {
		stopWobble(false);
		startWobbleAnimation(getPositionForID(getId()));
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mDropListener != null) {
				mDropListener.onListChange(start, stop);
				start = -1;
				stop = -1;
				started = false;
		...
		for (Long id : idList) {
			...
			if (view != null) {
				...
				if ((aboveRight(targetColumnRowPair, mobileColumnRowPair)
						...
						&& deltaYTotal > view.getTop()
						&& deltaXTotal < view.getRight() || above(
					...
					float xDiff = Math.abs(DynamicGridUtils.getViewX(view)
							- DynamicGridUtils.getViewX(mobileView));
					float yDiff = Math.abs(DynamicGridUtils.getViewY(view)
							- DynamicGridUtils.getViewY(mobileView));
	...
	public interface OnDropListener {
		...
		void onListChange(int originalPosition, int newPosition);
